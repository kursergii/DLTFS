...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps4000aSetSigGenArbitraryV2)(int16_t handle,
                                                                 int32_t offsetVoltage,
                                                                 uint32_t pkToPk,
                                                                 uint64_t startDeltaPhase,
...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps4000aSetSigGenBuiltInV2)(int16_t handle,
                                                               int32_t offsetVoltage,
                                                               uint32_t pkToPk,
                                                               PS4000A_WAVE_TYPE waveType,
                                                               double startFrequency,
...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps4000aSetSigGenPropertiesArbitraryV2)(int16_t handle,
                                                                           uint64_t startDeltaPhase,
                                                                           uint64_t stopDeltaPhase,
                                                                           uint64_t deltaPhaseIncrement,
                                                                           uint64_t dwellCount,
...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps4000aSetSigGenPropertiesBuiltInV2)(int16_t handle,
                                                                         double startFrequency,
                                                                         double stopFrequency,
                                                                         double increment,
                                                                         double dwellTime,
...
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps4000aSigGenFrequencyToPhaseV2)(int16_t handle,
                                                                     double frequency,
                                                                     PS4000A_INDEX_MODE indexMode,
                                                                     uint32_t bufferLength,
                                                                     uint64_t* phase);
...
// This function retreives the maximum value that can be returned by the driver for a probe connected to the specified channel at the given range.
// This will usually be the same as ps4000aMaximumValue, but may be lower depending on the probe and calibration.
//
// parameters:
// handle: identifies the device to which the probe is attached.
// channel: identifies the channel to which the probe is attached. If no probe is connected to this channel, an error will result.
// range: identifies what range the maximum value should be fetched for - the current set range for this channel is ignored.
// value: the maximum value the probe can return.
//
// return:
// PICO_OK
// PICO_INVALID_HANDLE (handle isn't that of an open PicoScope device).
// PICO_NOT_SUPPORTED_BY_THIS_DEVICE (device does not support probe interactions).
// PICO_PROBE_COLLECTION_NOT_STARTED (handle hasn't got a probe interaction callback set).
// PICO_INVALID_CHANNEL (channel doesn't map to a channel on the device with handle).
// PICO_INVALID_VOLTAGE_RANGE (the given range cannot be set for the currently connected probe).
// PICO_NULL_PARAMETER (value is NULL).
// PICO_DRIVER_FUNCTION (another thread is already calling a driver function).
PREF0 PREF1 PICO_STATUS PREF2 PREF3(ps4000aGetMaximumProbeValue)
(
	int16_t handle, 
	PS4000A_CHANNEL channel,
	PICO_CONNECT_PROBE_RANGE range,
